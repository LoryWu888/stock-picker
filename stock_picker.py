...
def score_stock(df_hist):
    ...
    total = trend + mom + vol_score + breakout + risk
...
# HTML 生成器（单文件自包含）
# ============================================================
def generate_html(payload, board_dict, dt_str):
...
<title>收盘后候选池 · 次日观察名单 ({dt_str})</title>
<style>
...
<header>
<h1>收盘后候选池 · 次日观察名单</h1>
<div class="sub">数据截至 {dt_str} · 共筛选 {len(payload)} 只 · 评分模型：趋势/动量/量能/突破/风控</div>
</header>
...
def main():
    ...
    print(f"[6/6] 生成候选池 Top {len(top)}...")
    
    dt_str = datetime.now().strftime('%m-%d %H:%M')
    html_content = generate_html(top, board_dict, dt_str)
